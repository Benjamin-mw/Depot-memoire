...
#### b) Méthode de Tukey combiné à la méthode d'Hampel
>>>>>>> bc8b119e08bb250ca1ee60d5a8b42469cf5aba48
...
summary(mod0_gamma_inv_Hampel)
```

```{r}
modBoth_gamma_inv_Both_1 = step(gamma.inv_Hampel_0, mod0_gamma_inv_Hampel, trace=F,direction = c('both'))
summary(modBoth_gamma_inv_Both_1)
```
...
```{r}
RMSE_train_Tukey_inv <- sqrt(mean(modBoth_gamma_inv_Tukey$residuals^2))
...
>>>>>>> c743d418662c960c2819b197c5c64661de2c5283
...
#### 1) Echantillon sans etude outliers / sans intéraction
...
#### 2) Echantillon sans etude outliers / avec intéractions
...
<<<<<<< HEAD
Ce message d'erreur apparaissant, nous allons davanate nous focalier sur l'échantillon train_Tukey
=======
...
```{r}
gamma.id_Hampel<- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact+RiskArea*MariStat,data = train_Hampel, family = Gamma(link = "identity"))
summary(gamma.id_Hampel)
```
...
```{r}
gamma.id_Hampel_Tukey<- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact+RiskArea*MariStat,data = train_Hampel_Tukey, family = Gamma(link = "identity"))
summary(gamma.id_Hampel_Tukey)
```
...
```{r}
mod0_gamma_id_Hampel_Tukey_0 <- glm(ClaimAmount ~ 1, data = train_Hampel_Tukey, family = Gamma(link = "identity"))
AIC(mod_gamma_id_2)
```

```{r}
modBoth_gamma_id_Both_Hampel_Tukey = step(gamma.id_Hampel_Tukey, mod0_gamma_id_Hampel_Tukey_0, trace=F,direction = c('both'))
AIC(modBoth_gamma_id_Both_Hampel_Tukey)
```
...
#### 1) Echantillon sans etude outliers / sans interactions
...
#### 2) Echantillon sans etude outliers / avec interactions
=======
...
#### 3) Etude avec intéractions et etude outliers : Tukey
...
```{r}
mod0_invgauss_mu_Tukey_1 <- glm(ClaimAmount ~ 1, data = train_Tukey, family = inverse.gaussian(link = "1/mu^2"))
AIC(mod0_invgauss_mu_Tukey_1)
```

```{r}
modBoth_invgauss_mu_Tukey_1 = step(invgauss.mu_Tukey, mod0_invgauss_mu_Tukey_1, trace=F,direction = c('both'))
summary(modBoth_invgauss_mu_Tukey_1)
```

##### RMSE

```{r}
RMSE_train_invgauss_1 <- sqrt(mean(modBoth_invgauss_mu_Tukey_1$residuals^2))
...
<<<<<<< HEAD
#### 2) Echantillon avec etude outliers : Tukey
=======
#### 2)Echantillon avec outliers
>>>>>>> 0f05214baa5642195365678910a3f2881cf982f3

```{r}
invgauss.id_Tukey <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Hampel_Tukey, family = inverse.gaussian(link = "identity"))

```

```{r}
<<<<<<< HEAD
invgauss.log_Tukey <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Hampel_Tukey, family = inverse.gaussian(link = "log"))
=======
...
AIC(mod_invgauss.inv_0)
=======
mod0_invgauss.inv_Tukey <- glm(ClaimAmount ~ 1, data = train_Tukey, family = inverse.gaussian(link = "inverse"))
summary(mod0_invgauss.inv_Tukey)
>>>>>>> c743d418662c960c2819b197c5c64661de2c5283
```

```{r}
mod_invgauss.inv_Both = step(invgauss.inv_Tukey, mod_invgauss.inv_0, trace=F,direction = c('both'))
AIC(mod_invgauss.inv_Both)
```
...
<<<<<<< HEAD
#### 1) Echantillon sans intéractions / ni outliers
...
```{r}
mod_lognorm.id_0 <- glm(log(ClaimAmount) ~ 1, data = train, family = gaussian(link = "inverse"))
AIC(mod_lognorm.id_0)
```

```{r}
mod_lognorm.id_Both_0 = step(lognorm.id_0, mod_lognorm.id_0, trace=F,direction = c('both'))
AIC(mod_lognorm.id_Both_0)
```

#### 2) Echantillon avec intéractions / ni outliers

```{r}
lognorm.id_1 <- glm(log(ClaimAmount) ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact+DrivAge_fact*RiskArea+MariStat*ClaimNbResp+RiskArea*MariStat, data = train, family = gaussian(link = "identity"))
summary(lognorm.id_1)
```

Les intéraction ne sont pas signifcatives pour ce modèle. Nous allons donc considérer un modèle sans intéraction.

#### 3) Echantillon sans intéraction, avec étude d'outliers : Tukey
=======
### A) Lien identité
...
```{r}
mod_lognorm.id_Tukey <- glm(log(ClaimAmount) ~ 1, data = train_Tukey, family = gaussian(link = "inverse"))
AIC(mod_lognorm.id_Tukey)
```

```{r}
mod_lognorm.id_Both_Tukey = step(lognorm.id_Tukey, mod_lognorm.id_Tukey, trace=F,direction = c('both'))
AIC(mod_lognorm.id_Both_Tukey)
```

### B) Lien Log

#### 1) Modèle sans outliers / sans intéractions
...
```{r}
mod_lognorm.log <- glm(log(ClaimAmount) ~ 1, data = train, family = gaussian(link = "log"))
AIC(mod_lognorm.log)
```

```{r}
mod_lognorm.log_Both = step(lognorm.log, mod_lognorm.log, trace=F,direction = c('both'))
AIC(mod_lognorm.log_Both)
```

#### 2) Avec outliers : Tukey

```{r}
lognorm.log_Tukey <- glm(log(ClaimAmount) ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Tukey, family = gaussian(link = "log"))
...
```{r}
mod_lognorm.log_Tukey <- glm(log(ClaimAmount) ~ 1, data = train_Tukey, family = gaussian(link = "log"))
AIC(mod_lognorm.log_Tukey)
```

```{r}
mod_lognorm.log_Tukey_Both = step(lognorm.log_Tukey, mod_lognorm.log_Tukey, trace=F,direction = c('both'))
AIC(mod_lognorm.log_Tukey_Both)
```
=======
...
modBoth_lognorm_id_Tukey_1 = step(lognorm.id_Tukey, mod0_lognorm_id_1, trace=F,direction = c('both'))
...
#### 1) Echantillon sans intéraction / sans outliers
...
```{r}
mod_lognorm.inv <- glm(log(ClaimAmount) ~ 1, data = train, family = gaussian(link = "inverse"))
AIC(mod_lognorm.inv)
=======
...
modBoth_lognorm_inv_Tukey_1 = step(lognorm.inv_Tukey, mod0_lognorm_inv_1, trace=F,direction = c('both'))
...
<<<<<<< HEAD
<<<<<<< HEAD
mod_lognorm.inv_Both = step(lognorm.inv, mod_lognorm.inv, trace=F,direction = c('both'))
AIC(mod_lognorm.inv_Both)
=======
lognorm.log <- glm(log(ClaimAmount) ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train, family = gaussian(link = "log"))
...
#### 2) Echantillon avec étude outliers, sans intéraction : Tukey

```{r}
lognorm.inv_Tukey <- glm(log(ClaimAmount) ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Tukey, family = gaussian(link = "inverse"))
summary(lognorm.inv_Tukey)
```
...
```{r}
mod_lognorm.inv_Tukey <- glm(log(ClaimAmount) ~ 1, data = train_Tukey, family = gaussian(link = "inverse"))
AIC(mod_lognorm.inv_Tukey)
```

```{r}
<<<<<<< HEAD
mod_lognorm.inv_Both_Tukey = step(lognorm.inv_Tukey, mod_lognorm.inv_Tukey, trace=F,direction = c('both'))
AIC(mod_lognorm.inv_Both_Tukey)
=======
modBoth_lognorm_id_0 = step(lognorm.id, mod0_lognorm_id_0, trace=F,direction = c('both'))
...
round(deviance(mod0_lognorm_log_Tukey_1))
  #modèle plein
round(deviance(invgauss.inv_Tukey))
  #modèle both

...
Nous avons commencé à enlever des valeurs extrêmes en limitant le domaine d'étude aux sinistres de montant inférieur à 25000€. Affinons cette recherche grâce à d'autres méthodes mises à notre disposition.
...
### II) GLM Lucas sans étude d'outliers
...
```{r}
mod0 <- glm(log(ClaimAmount) ~ 1, data = train, family = gaussian(link = "identity"))
summary(mod0)
```

```{r}
modBoth = step(lognorm.id, mod0, trace=F,direction = c('both'))
summary(modBoth)
```

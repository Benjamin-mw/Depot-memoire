...
### I) Etude des outliers

Nous avons commencé à enlever des valeurs extrêmes en limitant le domaine d'étude aux sinistres de montant inférieur à 20000€. Affinons cette recherche grâce à d'autres méthodes mises à notre disposition.
...
On va maintenant effectuer des GLM sur l'échantillon après détection d'outliers par la méthode de Tukey et Hampel

```{r}
gamma.inv_Hampel_Tukey <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Hampel_Tukey, family = Gamma(link = "inverse"),start =rep(0.01,28))
summary(gamma.inv_Hampel_Tukey)
```

```{r}
gamma.log_Hampel_Tukey <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Hampel_Tukey, family = Gamma(link = "log"))
summary(gamma.log_Hampel_Tukey)

gamma.id_Hampel_Tukey <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Hampel_Tukey, family = Gamma(link = "identity"))
summary(gamma.id_Hampel_Tukey)
```

#### c) Méthode de Tukey seule

Voyons si les résultats sont plus concluant avec la méthode de Tukey seule

```{r}
# Calculer l'écart interquartile (IQR)
iqr <- IQR(train$ClaimAmount)
...
Effectuons des GLM sur ces données

```{r}
gamma.log_Tukey <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Tukey, family = Gamma(link = "log"))
summary(gamma.log_Tukey)
```

```{r}
gamma.id_Tukey <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Tukey, family = Gamma(link = "identity"))
summary(gamma.id_Tukey)
```

Voyons sur l'écart de l'AIC entre le modèle réduit à l'intercept et le modèle complet est intéressant

```{r}
mod0_Tukey <- glm(ClaimAmount ~ 1, data = train_Tukey, family = Gamma(link = "log"))
summary(mod0_Tukey)
```

```{r}
modBoth_Tukey = step(gamma.log_Tukey, mod0_Tukey, trace=F,direction = c('both'))
summary(modBoth_Tukey)
```

### II) Modèle gamma.log_Tukey avec intéractions

```{r}
#On trace la matrice de corrélation des variables quantitatives
x <- train_Tukey[, c(1,2,9,11,12,13,14,15,16,17,19)]
corrplot(round(cor(x),2),method="ellipse")

```

```{r}
gamma.log_Tukey_1 <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact+DrivAge_fact*RiskArea+MariStat:ClaimNbResp 
+RiskArea*MariStat
,data = train_Tukey, family = Gamma(link = "log"))
summary(gamma.log_Tukey_1)
```

```{r}
summary(mod0_Tukey)

modBoth_Tukey_1 = step(gamma.log_Tukey_1, mod0_Tukey, trace=F,direction = c('both'))
summary(modBoth_Tukey_1)
```

### III) GLM Lucas sans étude d'outliers
...
#hist(prediction)
mod0 <- glm(ClaimAmount ~ 1, data = train, family = Gamma(link = "identity"))
```

```{r}
modBoth = step(gamma.id, mod0, trace=F,direction = c('both'))
summary(modBoth)
```
...
```{r}
modBack = step(gauss.id, mod0, trace=F,direction = c('backward'))
summary(modBack)
```
...
### I) Modèles avec étude des outliers

#### a) Méthode Tukey

```{r}
invgauss.mu_Tukey <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Tukey, family = inverse.gaussian(link = "1/mu^2"),start=rep(0.01,28))
summary(invgauss.mu_Tukey)
```

#### b) Méthode Tukey-Hampel

```{r}
invgauss.mu_Tukey_Hampel <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train_Hampel_Tukey, family = inverse.gaussian(link = "1/mu^2"),start=rep(0.01,28))
summary(invgauss.mu_Tukey_Hampel)
```

### II) Modele Lucas

```{r}
invgauss.mu <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train, family = inverse.gaussian(link = "1/mu^2"),start=rep(0.01,28))
summary(invgauss.mu)
```
...
```{r}
invgauss.id <- glm(ClaimAmount ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+RiskArea+BonusMalus+DrivAge_fact, data = train, family = inverse.gaussian(link = "log"),na.action = na.exclude)
```
...
```{r}
RMSE_train <- sqrt(mean(invgauss.mu_Tukey$residuals^2))
RMSE_train
```

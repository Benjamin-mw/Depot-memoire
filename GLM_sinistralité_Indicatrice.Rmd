...
freMPL5$DrivAge2 = freMPL5$DrivAge**2
```
...
```{r}
modFull <- glm(ClaimInd ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+Risque+BonusMalus+DrivAge_fact, data = train, family = binomial(link = log))
summary(modFull)
```
...
```{r}
modFull <- glm(ClaimInd ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+Risque+BonusMalus+DrivAge_fact, data = train, family = binomial(link = logit))
summary(modFull)
```
...
Ce modèle obtenu par une méthode de pas n'est pas tellement amélioré mais c'est celui que nous conserverons.
...
# Test avec une binomiale négative ?
...
On ne perd que 453 obersavtions avec ce procédé, ce qui représente moins de 1,8% des assurés.
...
```{r}
modFull_3 <- glm(Sinistres2 ~ MariStat+Categ+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+Risque+BonusMalus+DrivAge_fact, data = train3, family = poisson(link = "log"))
summary(modFull_3)
```
...
```{r}
residus = resid(modBoth_2)
```

## Visualisation des résidus

```{r}
plot(modBoth_2$fitted.values, residus, xlab = "Valeurs ajustées", ylab="Résidus", main = "Graphique en nuage de points des résidus par rapport aux valeurs ajustées")
plot(residus, type="h", ylab="Résidus standardisés", main = "Graphique en barres des résidus standardisés")
plot(resid(modBoth_2, type = "pearson"),type = "h", ylab="Résidus de Pearson", main="Graphique en barres des résidus de Pearson")
hist(residus)
```

## Vérification Résidus\~Poisson

Plusieurs tests existent pour vérifier si des variables aléatoires suivent une loi de Poisson. Nous allonc ici utiliser le test de Kolmogorov-Smirnov.

```{r}
#test de Kolmogorov-Smirnov
z = rpois(200, lambda = mean(modFull_2$residuals))
ks.test(residus, z)
```

Cela semble dire que les résidus ne suivent pas la loi de Poisson de paramètre la moyenne des résidus.

Cela pourrait éventuellement être dû à une moyenne des résidus trop élevée à cause de certaines observations...

Réalisons un test de Poisson :

```{r}
sum(modBoth_2$residuals<0)
sum(modBoth_2$residuals>=0)
```

```{r}
poisson.test(modBoth_2$residuals, conf.level = 0.95)
```

## Vérification pour une éventuelle surdispersion

```{r}
variance_obs = var(residus)
variance_att = mean(modBoth_2$fitted.values)
(rapport_variance = variance_obs/variance_att)
```

Alors, on a ici une surdispersion. Cela est clairement dû aux quelques valeurs extrêmes.

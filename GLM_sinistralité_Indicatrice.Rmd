...
z = rnorm(200, mean = mean(residus), sd=sqrt(var(residus)))
ks.test(residus,z)
```
...
```{r}
hist(residus, nclass = 100)
hist(residus, nclass = 500, xlim = c(-5,5))
```
...
```{r}
inf <- influence.measures(modBoth_2)
...
modFull_Outliers <- glm(Sinistres2 ~ MariStat+VehUsage+HasKmLimit+ClaimNbResp+ClaimNbNonResp+ClaimNbParking+ClaimNbFireTheft+ClaimNbWindscreen+OutUseNb+BonusMalus+Risque+DrivAge_fact*Categ, data = train_sansoutliers, family = poisson(link = "log"))
...
Ici, en retirant les 5 outliers, on reduit l'AIC de 4 unités. Ce n'est pas énorme mais nous conserverons ce modèle.

...
estimation_bin_logit <- predict(glm_tree_bin_logit, newdata = test)
...
```{r}
pred_bin_logit=prediction(estimation_bin_logit, test$ClaimInd)
pred_bin_logit
```

```{r}
perf_bin_logit=performance(pred_bin_logit,"tpr", "fpr")

auc_ROCR_bin_logit <- performance(pred_bin_logit, measure = "auc")
(auc_ROCR_bin_logit <- round(auc_ROCR_bin_logit@y.values[[1]],3) )
```

```{r}
summary(estimation_bin_logit)
summary(test$ClaimInd)
```
...
```{r}
score <- ifelse(predict(glm_tree_bin_logit,train,type="response") >.2, 1, 0)
...
```{r}
score <- ifelse(predict(glm_tree_bin_logit,test,type="response") >.2, 1, 0)
...
estimation_bin_log <- predict(glm_tree_bin_log, newdata = test)
mse_bin_log <- mean((estimation_bin_log - test$ClaimInd)^2)
mse_bin_log
```

```{r}
pred_bin_log=prediction(estimation_bin_log, test$ClaimInd)
pred_bin_logit
```

```{r}
perf_bin_log=performance(pred_bin_log,"tpr", "fpr")

auc_ROCR_bin_log <- performance(pred_bin_log, measure = "auc")
(auc_ROCR_bin_log <- round(auc_ROCR_bin_log@y.values[[1]],3) )
...
estimation_bin_probit <- predict(glm_tree_bin_probit, newdata = test)
mse_bin_probit <- mean((estimation_bin_probit - test$ClaimInd)^2)
mse_bin_probit
```

```{r}
pred_bin_probit=prediction(estimation_bin_probit, test$ClaimInd)
pred_bin_probit
```

```{r}
perf_bin_probit=performance(pred_bin_probit,"tpr", "fpr")

auc_ROCR_bin_probit <- performance(pred_bin_probit, measure = "auc")
(auc_ROCR_bin_probit <- round(auc_ROCR_bin_probit@y.values[[1]],3) )
```
...
```{r}
estimation_poi_log <- predict(glm_tree_poi_log, newdata = test)
